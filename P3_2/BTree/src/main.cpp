...
void createRelationForwardOfSize(int size);
void createRelationForwardWithinRange(int lowerBound, int upperBound);
void intTests();
void intTestsEmpty();
void intTestsLargeNum();
void intTestsNegative();
void intTestsSmNum();
void intTestsSameParam();
int intScan(BTreeIndex *index, int lowVal, Operator lowOp, int highVal, Operator highOp);
void indexTests();
void indexTestsOpt(int test_input);
void test1();
void test2();
void test3();
void test4();
...
void test4() {
    // Create a relation of size 0. Testing how program handles an empty tree.
    std::cout << "----------------------" << std::endl;
    std::cout << "createRelationForwardOfSize 0" << std::endl;
    createRelationForwardOfSize(0);
    // testing if the empty B+Tree functions properly
    indexTestsOpt(4);
    deleteRelation();
}

void test5() {
    //Create a relation whose integer values are all very large
    std::cout << "----------------------" << std::endl;
    std::cout << "createRelationForwardWithinRange" << std::endl;
    createRelationForwardWithinRange(1000000, 1000200);
    indexTestsOpt(5);
    deleteRelation();
}

void test6() {
    // Create a relation with negative integer values
    std::cout << "----------------------" << std::endl;
    std::cout << "createRelationForwardWithinRange" << std::endl;
    createRelationForwardWithinRange(-150, 50);
    indexTestsOpt(6);
    deleteRelation();
}

void test7() {
    //Create a relation with very few data entries, where the root stays un-split
    std::cout << "----------------------" << std::endl;
    std::cout << "createRelationForwardWithinRange" << std::endl;
    createRelationForwardWithinRange(100, 104);
    indexTestsOpt(7);
    deleteRelation();
}
...
/**
 * Create a forward relation with a given size constraint
 * @param size
 */
void createRelationForwardOfSize(int size) {
    std::vector<RecordId> ridVec;
...
/**
 * Create a forward relation where all integer values are within the given
...
/**
 * Test-Number-Specific index tests. The test_inpu corresponds to the text number
 * @param test_input
 */
void indexTestsOpt(int test_input)
{
...
/**
 * Integer test with an empty relation
 */
void intTestsEmpty()
{
    std::cout << "Create a B+ Tree index on the integer field" << std::endl;
...
/**
 * Integer test with integers of large numeric values.
 */
void intTestsLargeNum() {
    std::cout << "Create a B+ Tree index on the integer field" << std::endl;
    BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
    //1000000 ~ 1000200
    // run some tests
    checkPassFail(intScan(&index,1000000,GT,1000100,LT), 99)
    checkPassFail(intScan(&index,20,GTE,35,LTE), 0)
    checkPassFail(intScan(&index,-3,GT,3,LT), 0)
    checkPassFail(intScan(&index,996,GT,1001,LT), 0)
    checkPassFail(intScan(&index,0,GT,1,LT), 0)
    checkPassFail(intScan(&index,1000000,GT,1000010,LT), 9)
    checkPassFail(intScan(&index,3000,GTE,4000,LT), 0)
}

/**
 * Integer test with negative integers.
 */
void intTestsNegative()
{
    std::cout << "Create a B+ Tree index on the integer field" << std::endl;
    ...
    checkPassFail(intScan(&index,-150,GT,-100,LT), 49)
    checkPassFail(intScan(&index,0,GT,1,LT), 0)
    checkPassFail(intScan(&index,300,GT,400,LT), 0)
    checkPassFail(intScan(&index,3000,GTE,4000,LT), 0)
...
/**
 * Integer test with a small number of integers.
 */
void intTestsSmNum()
{
    std::cout << "Create a B+ Tree index on the integer field" << std::endl;
    ...
    checkPassFail(intScan(&index,20,GTE,35,LTE), 0)
    checkPassFail(intScan(&index,-3,GT,3,LT), 0)
    checkPassFail(intScan(&index,99,GT,103,LT), 3)
    checkPassFail(intScan(&index,100,GTE,102,LT), 2)
    checkPassFail(intScan(&index,100,GTE,102,LT), 2)
    checkPassFail(intScan(&index,300,GT,400,LT), 0)
...
/**
 * Integer test where a series of identical searches are made with the exact same parameter.
 */
void intTestsSameParam() {
    std::cout << "Create a B+ Tree index on the integer field" << std::endl;
    BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
